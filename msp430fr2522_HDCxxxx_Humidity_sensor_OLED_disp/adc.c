...
void adc_init_interrupt(void)
{
    ...
    ADCMCTL0 = ADC_CH_VBAT_MONITOR | ADCSREF_0;     // A13 ADC input select = 1.5V Ref
                                                    // Vref = DVCC
...
uint16_t adc_vbat_acquire(void)
{
    ...
    __bis_SR_register(LPM_LEVEL | GIE);                  // LPM0, ADC_ISR will force exit
    adc_val = ADCMEM0;
...
uint8_t vbat_level_checker(void)
{
    ...
    vbat_level = adc_vbat_acquire();
    __no_operation();
#if 1   // calculation method
#if 0   //2.5V ADC reference
    // detect voltage = 3.8V => 1024*(3.8/2)/2.5 = 778
    // detect voltage 3.7V => 758, 3.6V => 737, 3.5V => 737, 3.4V => 696, 3.3V => 676
    //vbat_decimal_x10 = (vbat_level*2) / 41;        // actual calculus : vbat_level / 20.48 assumed 20.5
    vbat_decimal_x10 = (vbat_level<<1) / 41;        // actual calculus : vbat_level / 20.48 assumed 20.5
#else   //3.3V ADC reference
        // detect voltage = 3.8V => 1024*(3.8/2)/3.3 = 590
    // detect voltage 3.7V => 574, 3.6V => 559, 3.5V => 543, 3.4V => 528, 3.3V => 512
    //vbat_decimal_x10 = (vbat_level*2) / 31;        // actual calculus : vbat_level / 15.51515 assumed 15.5
    vbat_decimal_x10 = (vbat_level<<1) / 31;        // actual calculus : vbat_level / 15.51515 assumed 15.5

#endif
#else   // direct mapping method
...
#endif
{
    switch(__even_in_range(ADCIV, ADCIV_ADCIFG))
    {
        ...
        case ADCIV_ADCIFG:
            ...
            __bic_SR_register_on_exit(LPM_LEVEL + GIE);// Exit from LPM
            break;
        default:
            break;
    }
