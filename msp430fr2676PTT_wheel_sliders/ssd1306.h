...
// select [#define CONST] if you want to store these data types at RAM
// select [#define CONST    const] if you want to store these data types at ROM
//#define CONST
#define CONST   const

#define LINE_BUF_SIZ     17                 // command(1byte) + buf_data
#define LCDWIDTH_PER_LINE_BUF_SIZ   9       // 128 / 16 = 8
...
// activate this feature if the memories for the fonts are not enough..
#define FONT_MEMORY_SAVE        1
...
typedef struct {
    uint8_t width;
    uint8_t height;
    uint8_t data_type;           // type : compressed, raw
    const uint8_t *data;
