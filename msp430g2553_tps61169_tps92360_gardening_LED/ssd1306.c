...
void setAddress( char page, char column )
{
    ...
    unsigned char pageAddress[] = {SSD1306_PAGE_START_ADDRESS};
    unsigned char columnAddress[] = { SSD1306_COLUMNADDRESS, SSD1306_COLUMNADDRESS_MSB, SSD1306_COLUMNADDRESS_LSB };
...
void ssd1306_printString(uint8_t x, uint8_t y, char *ptString, font_idx font_id)
{
    ...
    while (*ptString != '\0')
    {
        ...
        if ((x + fonts[font_id].width) >= 127) {                  // exception if x is over the range then position will go to the next line(y)
            x = 0;

            y++;
            if((x + fonts[font_id].height) > 8)
                y++;
            if((x + fonts[font_id].height) > 16)
                y++;
        ...
        if(fonts[font_id].height <= 8)
        {
            ...
            for(i = 0; i< fonts[font_id].width; i++) {
                ...
                buffer[i+1] = *(fonts[font_id].data + (*ptString-0x20)*fonts[font_id].width +i);     // ' ' means space bar and ASCII code is 0x20
            }

            buffer[fonts[font_id].width + 1] = 0x0;    // at the end of buffer = 0x00

            i2c_write(SSD1306_I2C_ADDRESS, buffer, 7);
            x += fonts[font_id].width+2;                // x position
        ...
        else if( (fonts[font_id].height > 8) && (fonts[font_id].height <= 16) )
        {
            ...
            for(i = 0; i< fonts[font_id].width; i++) {
                ...
                buffer[i+1] = *(fonts[font_id].data+(*ptString-0x20)*fonts[font_id].width*2 +(i*2) );     // ' ' means space bar and ASCII code is 0x20
            }
            buffer[fonts[font_id].width + 1] = 0x0;     // at the end of buffer = 0x00
            i2c_write(SSD1306_I2C_ADDRESS, buffer, fonts[font_id].height+1);

            buffer[0] = 0x40;
            ssd1306_setPosition(x+2, y+1);                                  // y+1 position
            for(i = 0; i< fonts[font_id].width; i++) {
                //buffer[i+1] = fonts[font_id].data[*ptString - ' '][2*i+1];
                buffer[i+1] = *(fonts[font_id].data+(*ptString-0x20)*fonts[font_id].width*2 +(i*2+1) );
            }
            buffer[fonts[font_id].width + 1] = 0x0;     // at the end of buffer = 0x00
            i2c_write(SSD1306_I2C_ADDRESS, buffer, fonts[font_id].height+1);

            x += fonts[font_id].width+2;
        }
        else if( (fonts[font_id].height > 16) && (fonts[font_id].height <= 24) )
        {
        }
        else
        {
